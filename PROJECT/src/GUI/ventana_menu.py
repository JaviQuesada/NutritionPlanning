...
class VentanaMenu:
    ...
    def __init__(self, root, menu, datos_dia, objetivo_calorico, ventana_preguntas):
        ...
        self.root.minsize(1024, 768)  # Tamaño mínimo y de apertura aumentado
        ...
        for col, dia in enumerate(constantes.DIAS_SEMANA):
            lbl = ttk.Label(frame, text=dia, font=("Helvetica", 12, "bold"), borderwidth=1, relief="solid")
        ...
        for row, comida in enumerate(constantes.COMIDAS):
            lbl = ttk.Label(frame, text=comida["nombre"], font=("Helvetica", 12, "bold"), borderwidth=1, relief="solid")
        ...
        # Añade objetivo calorico y calorias/micronutrientes en la misma fila
        objetivo_lbl = ttk.Label(frame, text=f"Objetivo calórico: {objetivo_calorico:.2f}", font=("Helvetica", 10, "bold"), borderwidth=1, relief="solid", wraplength=120)
        ...
        for col, dia in enumerate(constantes.DIAS_SEMANA):
            ...
            lbl = ttk.Label(frame, text=resumen, font=("Helvetica", 10, "bold"), borderwidth=1, relief="solid", wraplength=120)
        ...
        ttk.Button(button_frame, text="Atrás", command=self.volver).pack(side="left", padx=(50, 20), expand=True)
        ttk.Button(button_frame, text="Cerrar", command=self.cerrar).pack(side="left", padx=(20, 50), expand=True)
