#algoritmo_genetico_penalizacion_dinamica1
...
def restriccion_calorias(calorias_diarias, objetivo_calorico, generacion):
    ...
    if calorias_diarias < limite_inferior or calorias_diarias > limite_superior:
        penalizacion_calorias = abs(objetivo_calorico - calorias_diarias) * (PENALIZACION_CALORIAS * generacion) ** ALPHA
    else:
...
def restriccion_macronutrientes (calorias_diarias, proteinas_diarias, carbohidratos_diarias, grasas_diarias, generacion):
    ...
    return penalizacion_macronutrientes * (PENALIZACION_MACRONUTRIENTES * generacion) ** ALPHA
...
def restriccion_alergia(alimento, grupos_alergia, generacion):

    if alimento["grupo"] in grupos_alergia:
        return (PENALIZACION_ALERGIA * generacion) ** ALPHA
    else:
        return 0
            ...
            for dia in range(NUM_DIAS):
                ...
                for indice_comida, comida in enumerate(COMIDAS):
                    ...
                    for indice_alimento in range(num_alimentos):
                        ...
                        total_penalizaciones_alergia += restriccion_alergia(alimento, self.grupos_alergia, self.generacion)
                ...
                penalizacion_objetivo_calorias = restriccion_calorias(calorias_diarias, self.objetivo_calorias, self.generacion)
                ...
                penalizacion_objetivo_macronutriente = restriccion_macronutrientes(calorias_diarias, proteinas_diarias, carbohidratos_diarias, grasas_diarias, self.generacion)
...
def ejecutar_algoritmo_genetico(comida_basedatos, objetivo_calorico, grupos_alergia, grupos_gusta, grupos_no_gusta):
    ...
    def actualizar_generacion (algoritmo):
        problema.generacion = algoritmo.n_gen
    ...
    resultado = minimize(
        ...
        verbose=True,
        save_history=True,
        callback = actualizar_generacion
    )
